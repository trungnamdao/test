...
function animate() {
    ...
    //  0.75         AUTO EXPLODE 💥
    //  0.75 → 0.87  Hologram dots (particles → mockup image shape)
    //  0.87 → 1.00  #final-screen HTML fade-in (canvas fades out)
    // ──────────────────────────────────────────────────────────────────────

    // Điều chỉnh Bloom và Particle Size theo phase
    // Khi nổ và làm hologram (0.75 -> 0.95), giảm bloom để không bị chói trắng
    const mockupLerp = Math.max(0, Math.min(1, (smoothProgress - 0.75) / 0.12));
    if (mockupLerp > 0) {
        bloomPass.strength = 1.2 - mockupLerp * 0.6; // Giảm từ 1.2 xuống 0.6
        if (particles) particles.material.size = 0.12 - mockupLerp * 0.04; // Giảm size xuống 0.08
    } else {
        bloomPass.strength = 1.2;
        if (particles) particles.material.size = 0.12;
    }
    ...
    // Scroll indicator
    if (scrollInd) {
        const showScrollCue = smoothProgress < 0.87;
    ...
    if (particles && geometry) {
        ...
        // Phase 3: Hologram dots (0.75 → 0.87)
        const ml   = Math.max(0, Math.min(1, (smoothProgress - 0.75) / 0.12));
